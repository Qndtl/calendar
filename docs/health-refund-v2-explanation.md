...
**공제 정당 조건**: conditionA 하나만 사용합니다.

**conditionA**: `sorted3[0]`이 초일(1일) AND (`sorted3[last]`이 말일 OR `sorted2.length > 0`)

- 초일 출역 = "이 달부터 근무 시작"의 증거
- 말일 출역 or sorted2 출역 = "끊김 없이 이어짐"의 증거
- 초일 출역이 없으면 나머지 조건과 무관하게 무조건 **환급**
